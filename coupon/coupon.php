...
$couponCount = $resultAll->num_rows; //所有的使用者
...
$sql = "SELECT * FROM coupon WHERE valid=1 ORDER BY $orderType LIMIT $start, $perPage";
$result = $conn->query($sql);
$pageCouponCount = $result->num_rows; //每頁的使用者
...
// $totalPage;
// $quotient = floor($couponCount / $perPage);
...
<body>
  ...
  <main class="main-content p-4">
    <div class="d-flex justify-content-between align-items-center border-bottom border-dark border-5 pb-2 mb-3">
      <h1><i class="fa-solid fa-ticket me-3"></i>所有優惠券</h1>
    </div>
    <div class="container">
      <div class="row justify-content-between align-items-center mb-3">
        <div class="col-md-3">
          <div class="py-2">
            ...
            <button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#staticBackdrop">
              <i class="fa-solid fa-circle-plus me-2"></i>新增優惠券
            </button>
              ...
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">新增優惠券</h5>
                    ...
                    <div class="container">
                      <form action="doCreate.php" method="post">
                        ...
                        <div class="mb-2">
                          ...
                          <input type="date" class="form-control" name="time_end" required>
                        </div>
        ...
        <div class="col-md-4">
          <div class="py-2 d-flex justify-content-end align-items-center">
            ...
            <div class="btn-group">
              <a href="coupon.php?page=<?= $page ?>&order=1" class="btn btn-info
        <?php if ($order == 1) echo "active"; ?>"><i class="fa-solid fa-arrow-down-short-wide me-2"></i>編號</a>
              <a href="coupon.php?page=<?= $page ?>&order=2" class="btn btn-info
      ...
      <div class="py-2 border-top">
        第<?= $startItem ?>-<?= $endItem ?> 筆，共<?= $couponCount ?>筆資料
      </div>
      <div class="py-2">
        <nav aria-label="Page navigation example">
          <ul class="pagination pagination-sm justify-content-center">
            <li class="page-item"><a class="page-link" href="coupon.php?page=<?php
                                                                              if ($page - 1 <= 0) {
                                                                                echo 1;
                                                                              } else {
                                                                                echo $page - 1;
            ...
            "><a class="page-link" href="coupon.php?page=<?= $i ?>&order=<?= $order ?>"><?= $i ?></a></li>
            <?php endfor; ?>
            <li class="page-item"><a class="page-link" href="coupon.php?page=<?php
                                                                              if ($page + 1 > $totalPage) {
                                                                                echo $totalPage;
          ...
          <tbody>
            ...
            while ($row = $result->fetch_assoc()) : ?>
              <tr>
                <td><?= $row["id"] ?></td>
                <td><?= $row["name"] ?></td>
                <td><?= $row["time_start"] ?> <span class="mx-2">~</span> <?= $row["time_end"] ?></td>
                <td><?= $row["create_time"] ?></td>
                <td class="text-end text-danger">$<?= $row["discount"] ?></td>
                <td class="text-center">
                  <a href="coupon-detail.php?id=<?= $row["id"] ?>" class="btn btn-sm btn-primary"><i class="fa-solid fa-eye me-2"></i>詳細資料</a>
                  <a href="coupon-edit.php?id=<?= $row["id"] ?>" class="btn btn-sm btn-warning"><i class="fa-solid fa-pen-to-square me-2"></i>修改</a>
